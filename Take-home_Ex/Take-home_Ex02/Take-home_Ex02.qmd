...
-   Extract nodes. Check source and target companies, categorize them into "fisher" or "wholesaler".
...
```{r}
lookup1 <- mc2_nodes_hscode1 %>%
  group_by(id) %>%
  summarise(count=n()) %>%
  filter(count>1) %>%
  left_join(mc2_edges_hscode1%>%
              group_by(from)%>%
...
mc2_nodes_hscode1 <- mc2_nodes_hscode1 %>%
  left_join(lookup1%>%select(id,group),by="id") %>%
...
-   Find nodes with between-ness \>= 5:

```{r}
mc2_graph_hscode1 %>%
  mutate(centrality_bet = centrality_betweenness()) %>%
  filter(centrality_bet>=5) %>%
  select(id,group)
```
...
-   Find edges of hscode equal to "940360".
...
```{r}
lookup2 <- mc2_nodes_hscode2 %>%
  group_by(id) %>%
  summarise(count=n()) %>%
  filter(count>1) %>%
  left_join(mc2_edges_hscode2%>%
              group_by(from)%>%
              summarise(from_weights= sum(weights)),
            by=c("id"="from")) %>%
  left_join(mc2_edges_hscode2%>%
...
mc2_nodes_hscode2 <- mc2_nodes_hscode2 %>%
  left_join(lookup2%>%select(id,group),by="id") %>%
  mutate(group = ifelse(!is.na(group.y),group.y,group.x)) %>%
  select(-group.y,-group.x) %>%
  distinct()
```
...
-   Find edges of hscode equal to "870899".
...
mc2_nodes_hscode3 <- mc2_nodes_hscode3 %>%
  left_join(lookup3%>%select(id,group),by="id") %>%
  mutate(group = ifelse(!is.na(group.y),group.y,group.x)) %>%
  select(-group.y,-group.x) %>%
  distinct()
```
